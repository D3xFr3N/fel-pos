...
def _write_restart_script(
    ...
    *,
    ...
    lines = [
        ...
        "    try {",
        "      Write-UpdateLog \"Relanzando FELPOS.exe directo\"",
        "      Start-Process -LiteralPath $exe -WorkingDirectory $InstallDir | Out-Null",
        "      $launched = $true",
        "    } catch {",
        "      Write-UpdateLog (\"AVISO Start-Process EXE: \" + $_.Exception.Message)",
        "    }",
        "  }",
        "  if (-not $launched -and (Test-Path -LiteralPath $iniciarVbs)) {",
        "    try {",
        "      Write-UpdateLog \"Relanzando via Iniciar_FELPOS.vbs\"",
        "      Start-Process -FilePath 'wscript.exe' -ArgumentList @('//nologo', $iniciarVbs) -WorkingDirectory $InstallDir | Out-Null",
        "      $launched = $true",
        "    } catch {",
        "      Write-UpdateLog (\"AVISO Iniciar_FELPOS.vbs: \" + $_.Exception.Message)",
        "    }",
        "  }",
        "  if (-not $launched -and (Test-Path -LiteralPath $RelaunchVbs)) {",
        "    try {",
        "      Write-UpdateLog \"Relanzando via $RelaunchVbs\"",
        "      Start-Process -FilePath 'wscript.exe' -ArgumentList @('//nologo', $RelaunchVbs) | Out-Null",
        "      $launched = $true",
        "    } catch {",
        "      Write-UpdateLog (\"AVISO relaunch.vbs: \" + $_.Exception.Message)",
        "    }",
        "  }",
        "  if (-not $launched) {",
        "    Write-UpdateLog 'ERROR: no se pudo abrir FEL POS automaticamente'",
        "  } else {",
        "    Write-UpdateLog 'FEL POS abierto automaticamente'",
        "  }",
        "}",
        "",
